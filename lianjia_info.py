...
def getresp(req,retries = 2):
    ...
    try:
        ...
        ses.get('http://sz.lianjia.com/xiaoqu/rs/')
...
for x in xiaoqus:
    if x:
        ...
        if not tab2.find_one({'url':url}):
            ...
            rel = [r'<h1 class="detailTitle">(.*?)</h1><div class="detailDesc">(.*?)</div>[\s\S]*?<a href="/xiaoqu/">(.*?)</a>[\s\S]*?<a href=[\s\S]*?>(.*?)</a>[\s\S]*?<a href=[\s\S]*?>(.*?)</a>[\s\S]*?'
                   + r'[\s\S]*?<div class="xiaoquPrice clear"><div class="fl">((暂无均价信息)|(<span class="xiaoquUnitPrice">.*?</span>元/㎡))</div></div>[\s\S]*?'
                   # + r'[\s\S]*?<div class="xiaoquPrice clear"><div class="fl">(.*?)</div></div>[\s\S]*?'
                   + r'<span class="xiaoquInfoLabel">' + u"建筑年代" + '</span><span class="xiaoquInfoContent">(.*?)</span></div>'
                   + r'<div class="xiaoquInfoItem"><span class="xiaoquInfoLabel">' + u'建筑类型' + '</span><span class="xiaoquInfoContent">(.*?)</span></div>'
                   + r'<div class="xiaoquInfoItem"><span class="xiaoquInfoLabel">' + u'物业费用' + '</span><span class="xiaoquInfoContent">(.*?)</span></div>'
                   ]
            ...
            key[u'区'] = xiaoqu[0][3]
            key[u'道路'] = xiaoqu[0][4]
            key[u'小区均价'] = xiaoqu[0][5]
            key[u'建成时间'] = xiaoqu[0][6]
            key[u'建筑类型'] = xiaoqu[0][7]
